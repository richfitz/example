...
# install example as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `example`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d example.XXXXXX)
git clone git@github.com:richfitz/example .
...
Visit
[https://test.pypi.org/project/example](https://test.pypi.org/project/example)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d example-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple example
```
...
Don't forget to also make a [release on GitHub](https://github.com/richfitz/example/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.